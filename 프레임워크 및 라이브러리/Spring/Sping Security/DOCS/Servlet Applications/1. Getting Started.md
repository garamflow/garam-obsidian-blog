...
- 만약 위의 사례들이 찾고 있는 것과 일치하지 않는다면, 다음과 같은 순서로 애플리케이션을 고려해보자.
	- **프로토콜**: 먼저, 애플리케이션이 사용할 통신 프로토콜을 결정하자.
		- 서블릿 기반 애플리케이션의 경우, Spring Security는 HTTP와 웹소켓을 모두 지원한다.
	- **인증**: 다음으로, 사용자가 어떻게 인증할 것인지, 그리고 그 인증이 상태 유지(stateful)인지 무상태(stateless)인지 고려해보자.
	- **권한 부여**: 그 다음, 사용자가 수행할 수 있는 작업을 어떻게 결정할 것인지 생각해 보자.
	- **방어**: 마지막으로, Spring Security의 기본 보호 기능을 통합하고, 어떤 추가 보호 기능이 필요한지 검토해보자.