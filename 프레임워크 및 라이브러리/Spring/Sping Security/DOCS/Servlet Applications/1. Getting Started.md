...
- Spring Boot와 Spring Security의 기본 구성은 런타임에 다음과 같은 동작을 제공한다.
	- [모든 엔드포인트](https://docs.spring.io/spring-security/reference/servlet/authorization/authorize-http-requests.html)(Boot의 `/error` 엔드포인트 포함)에 대해 인증된 사용자를 요구한다.
	- 시작 시 생성된 비밀번호로 [기본 사용자를 등록](https://docs.spring.io/spring-security/reference/servlet/authentication/passwords/user-details-service.html)한다.
		- 비밀번호는 콘솔 로그에 출력된다.
	- [BCrypt 및 기타 방식으로 비밀번호 저장](https://docs.spring.io/spring-security/reference/servlet/authentication/passwords/password-encoder.html)을 보호한다.
	...
	- CSRF 공격을 완화한다.
	- 세션 고정 공격을 완화한다.
	- HTTPS 사용을 보장하기 위해 Strict-Transport-Security 헤더를 작성한다.
	- 스니핑 공격을 완화하기 위해 X-Content-Type-Options 헤더를 작성한다.
	- 인증된 리소스 보호를 위해 Cache Contorl 헤더를 작성한다.
	- 클릭재킹을 완화하기 위해 X-Frame-Options 헤더를 작성한다.
	- `HttpServletRequest`의 인증 메서드와 통합된다.
	- 인증 성공 및 실패 이벤트를 발생시킨다.