...
- JPA는 지연 로딩과 즉시 로딩 방식을 제공하는데, `@EntityGraph`를 사용하면 이런 로딩 전략을 쿼리 메서드 수준에서 동적으로 변경 가능하다.

## 2) 사용 방법
- 레포지토리 인터페이스의 메서드 위에서 선언해서 사용한다.
...
- 기본적으로 `Author` 는 지연 로딩으로 설정되어 있지만 `findById` 메서드에 `@EntityGraph`를 사용해서 `author`를 즉시 로딩하게 지정되어 있다.
- 해당 메서드를 통해 `Book` 엔티티를 조회할 때 `Author` 엔티티도 함께 로딩된다.
