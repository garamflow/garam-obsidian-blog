...
### **3. SimpleJobBuilder API (Spring Batch 5.x 기준)**
...
### **4. SimpleJob 실행 예시 (Spring Batch 5.x 기준)**
...
### **5. SimpleJob API 요약**
- **start(Step)**: 첫 번째 **Step**을 설정.
- **next(Step)**: 다음 **Step**을 순차적으로 설정.
- **incrementer(JobParametersIncrementer)**: **JobParameters** 값을 자동으로 증가시킴.
- **preventRestart(boolean)**: **Job**의 재시작 가능 여부 설정.
- **validator(JobParametersValidator)**: **JobParameters** 검증.
- **listener(JobExecutionListener)**: **Job**의 실행 전/후에 콜백을 설정.
- **build()**: 설정된 **Job**을 최종 생성.

---

### **Spring Batch 5.x 버전의 주요 변경 사항**
- **JobBuilderFactory** 및 **SimpleJobBuilder**는 여전히 사용되지만, **JobRepository**를 기반으로 **Job**과 **Step**을 설정하는 방식이 더욱 강조됩니다.
- **JobBuilderFactory**는 **JobRepository**를 사용하여 **JobBuilder**를 반환하고, 이를 통해 **SimpleJob**을 구성합니다.
