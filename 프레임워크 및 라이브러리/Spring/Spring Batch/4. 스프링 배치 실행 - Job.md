...
```java
public Job batchJob() { return jobBuilderFactory.get(“batchJob") 
	.start(Step) // 처음 실행 할 Step 설정, 최초 한번 설정, SimpleJobBuilder 가 생성되고 반환된다
	.next(Step) // 다음에 실행 할 Step 들을 순차적으로 연결하도록 설정 // 여러 번 설정이 가능하며 모든 next() 의 Step 이 종료가 되면 Job 이 종료된다
	.incrementer() .validator() .preventRestart() .listener() .build(); }
```

Builder 부분 -> Job의 step 설정 -> 실행하면 JobLauncher가 Job 생성 -> step 클래스의 tasklet 부분 -> step1, step2, ... 설정
