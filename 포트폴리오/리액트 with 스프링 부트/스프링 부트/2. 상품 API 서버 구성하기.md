...
- 연관관계에서 `ToString`을 조심해야한다.
	- `@ToString(exclude = "imageList")`
		...
		- 이럴때는 트랜잭션이 필요하지 않다.
		- exclude를 하지 않으면 `Product`에서 한 번, `ProductImage`에서 한 번, 총 두 번 갖고오게 된다.
...
## 11) 이미지가 포함된 목록 처리하기
- 상품과 이미지를 같이 가지고 올 때, 목록 부분에서 주의해야한다.
	- 상품 10개짜리 리스트 하나를 갖고 온다고 치면 한 세트를 갖고 올 때 한 번, 상품 하나씩 읽어오는데 한 번씩 총 10번의 쿼리를 날리게 되면 DB가 바로 뻗을 수 있다.
	- 그래서 @ElementCollection와 Join을 적절히 사용해줘야한다.
```java
// ProductRepositoryTwo
