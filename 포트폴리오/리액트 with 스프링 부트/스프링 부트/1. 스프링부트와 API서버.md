...
```
- ProductSearch 인터페이스에서 `search1()` 메소드에 대해서도 작성해주자.
- querydsl을 이용해서 사용하는 방식이다.
	- 인터넷 검색 시 나오는 코드들이 예전 방식인 경우도 많고, 변화가 빠른 편이다.
```java
@Override  
public Page<ProductEntity> search1() {  
...
- 동적 쿼리를 위해서 필요한 데이터는 DTO로 받는다.
- 서비스 계층은 기본적으로 파라미터와 리턴은 DTO가 맞다고 보는 시각도 있다.
	- 특히 JPA는 엔티티는 JPA에서 관리를 하고 있는 객체로, db라고 생각하는게 좋다.
...
- DTO를 만들 때 기본적으로 일단 똑같이 만들면 되는데, 변경이 자유로우므로 getter, setter를 다 만들어준다.
	- 그리고 엔티티는 테이블과 관련 있으나 DTO는 관련 없다.
	- 오히려 업무와 관련 있다. 상황에 따라 여러개를 만들어도 된다.
- DTO를 엔티티로 바꾸고 엔티티를 DTO로 바꾸는 경우가 생길 수 있다.
	- 그것을 서비스 계층에서 변환하는 작업을 하는게 좋다.
