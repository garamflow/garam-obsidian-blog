...
## 8) 등록하기
- `ProductService`에 새로운 데이터를 등록에 관한 기능을 만들어줄 것이다.
- 등록을 할 때, 방법은 두 가지가 있다.
	- 새로운 등록되는 정보 그 자체를 리턴해주는 방법
	- PK가 몇 번으로 만들어졌는지를 확인하고 리턴해주는 방법
```java
public interface ProductService {
...
- `ProductService`에 코드를 추가하면 당연히 `ProductServicpeImpl`에도 Override 해줘야한다.
```java
public class ProductServiceImpl implements ProductService {
...
public void testRegister() {
    ProductDTO productDTO = ProductDTO.builder()
            ...
            .price(110.0)
            .stockQuantity(3)
            .ImageUrl("test service")
            .uploadDate(LocalDate.parse("2020-10-11"))
            .build();

    log.info(productService.registerProduct(productDTO));
}
```

## 9) 수정하기
- 수정하기는 조심해야되는게 엔티티, 다시말해서 데이터베이스를 가지고 와서 바꿔줘야된다.
- id를 통해서 데이터를 가지고 오고서, change를 이용해 데이터 값을 바꿔줘야 한다. 
...
public void modifyProduct(ProductDTO productDTO) {
    Optional<ProductEntity> result = productRepository.findById(productDTO.getId());
    ProductEntity productEntity = result.orElseThrow();

    productEntity.setName("change name") 
    productEntity.setStockQuantity(4);
    productEntity.setPrice(150.0);
    productEntity.setImageUrl("test");
    productEntity.setDescription("change des");
    productEntity.setUploadDate(LocalDate.parse("2020-10-15"));
...
## 11) 페이징 처리 DTO 만들기
- 페이징 처리에 관해서 DTO를 만들어서 처리하면 더 간편하게 만들 수 있다.
- 모든 페이징에 필요한 곳에서 사용할 수 있다.
- `package org.mall.shoppingmallapi.dto;`
### (1) `PageRequestDTO`
```java
package org.mall.shoppingmallapi.dto;
...
### (2) `PageResponseDTO`
- 페이징 처리의 결과물은 보통 두 가지 종류이다.
	- 실제 데이터
	- 데이터 목록
- 페이지 처리 시, 페이지 번호를 계산하는 것도 중요하다.
	- 12페이지이면 11~20페이지가 보여줘야 한다.
	- 데이터가 78개 있으면 8페이지를 보여줘야 한다.
```java
package org.mall.shoppingmallapi.dto;
...
## 12) 페이징 처리 서비스에 적용하기
- 페이징은 검색과 연관이 있으므로 검색쪽과 service 관련 코드를 수정해줘야한다.
```java
public interface ProductSearch {
    Page<ProductEntity> searchProduct(PageRequestDTO pageRequestDTO);
}
```
...
public class ProductSearchImpl extends QuerydslRepositorySupport implements ProductSearch {
    ...
    @Override
    public Page<ProductEntity> searchProduct(PageRequestDTO pageRequestDTO) {  
        ...
        // 쿼리 실행 (목록 데이터 가져오기)  
        List<ProductEntity> list = query.fetch();  
...
public PageResponseDTO<ProductDTO> getList(PageRequestDTO pageRequestDTO) {
    ...
    // JPA
    Page<ProductEntity> result = productRepository.searchProduct(pageRequestDTO);

    // ProductEntity List -> ProductDTO List
    List<ProductDTO> dtoList = result  
            .get()
            .map(productEntity -> entityToDTO(productEntity)).collect(Collectors.toList());

    PageResponseDTO<ProductDTO> responseDTO =  
            PageResponseDTO.<ProductDTO>withAll()
            .dtoList(dtoList)
...
## 13) REST 방식 컨트롤러 만들기
- Pathvariable
	- `/api/todo/33` 은 누구나 동일한 화면을 보게된다.
- queryString
	- `/list?page=3` 은 상황에 따라 다른 결과가 나온다.
	- 이런 경우는 queryString을 사용한다.
...
@PostMapping("/")
public Map<String, Long> registerProduct(@RequestBody ProductDTO productDTO) {
    log.info("productDTO" + productDTO);

    Long id = productService.registerProduct(productDTO);
...
### (2) formatter
- 파라미터를 처리할 때, 귀찮은 것들이 날짜, 불린값, 파일 등이 있다.
	- 이런것들은 포맷터 처리를 해주면 더 쉬워진다.
	- 모든 날짜 처리를 날짜 포맷터로 처리하기
```java
package org.mall.shoppingmallapi.controller.formatter;
...
- 포맷터를 만들어주고 현재 프로젝트에서 사용하게 등록해줘야한다.
	- 그러려면 configuration을 등록해줘야한다.
```java
package org.mall.shoppingmallapi.config;

import org.mall.shoppingmallapi.controller.formatter.LocalDateFormatter;
...
```java
@PutMapping("/{id}")  
public Map<String, String> modifyProduct(@PathVariable("id") Long id,  
...
### (4) CORS 설정
- controller에서 직접 설정하는 방법도 있지만 좀 귀찮아진다.
- 그래서 웹과 관련된 설정을 할 때 전체적으로 적용되게 해주는게 좋다.
```java
@Configuration
public class CustomServletConfig implements WebMvcConfigurer {
	...
...
```
- `addCorsMappings`
	- CORS 정책을 설정하기 위한 메소드
	- `CorsRegistry` 객체를 파라미터로 받아서 해당 객체를 이용해 정책을 구성한다.
- `addMapping`
	- 어떤 경로에 CORS 정책을 적용할 것인가
	- `/**` 는 애플리케이션의 모든 URL에 대해 CORS 정책을 적용한다.
- `maxAge`
	- 브라우저가 사전 요청의 결과를 캐싱할 수 있는 최대 시간
- `allowedMethods`
	- 허용되는 HTTP 메소드를 설정한다.
	- `OPTIONS`는 pre-flight를 위해서 허용해줘야한다.
- `allowedOrgins`
	- 모든 출처에서의 요청을 허용한다.

## 14) RestControllerAdvice
- RestControllerAdvice는 RestController가 문제가 생겼을 때, 예외처리를 하는 것이다.
- 공통적으로 나오는 문제 사항을 처리할 수 있다.
- 익셉션 핸들러가 가장 중요하다.
```java
package org.mall.shoppingmallapi.controller.advice;
